...
export default async function AboutPage() {
    ...
    try {
        ...
        console.error('DB error on about content fetch', err)
    }
        ...
        <div className="min-h-screen">
            ...
            <Section variant="dark" className="pt-32 pb-24 text-center">
                <div className="max-w-4xl mx-auto space-y-8 animate-fade-up">
                    ...
                    <h1 className="text-6xl md:text-8xl lg:text-9xl font-serif text-white tracking-tighter leading-none">
                        {getContent('about_hero_title', "WHO WE ARE.")}
                    </h1>
                    <p className="text-2xl md:text-3xl font-serif text-brand-yellow/80 max-w-2xl mx-auto">
                        {getContent('about_hero_subtitle', '"Mugambo Wa Ma" — The Voice of Truth')}
                    </p>
                </div>
                ...
                <div className="grid lg:grid-cols-2 gap-20 items-center">
                    <div className="space-y-8">
                        ...
                        <h2 className="text-4xl md:text-6xl font-serif text-brand-black leading-tight">
                            {getContent('about_establishment_title', "Broadcasting with Purpose.")}
                        </h2>
                        <div className="prose prose-xl prose-gray font-medium text-gray-600 leading-relaxed">
                            <p>
                                {getContent('about_establishment_content', 'Muoroto FM is a leading National Radio Station based in the heart of Nairobi County, Kenya. We serve as a vital link between traditional Gikuyu culture and modern Kenyan aspirations.')}
                            </p>
                        </div>
                    </div>
                    ...
                    {pillars.map((pillar, i) => (
                        <div key={i} className="group p-10 bg-white rounded-[2.5rem] shadow-xl border border-border/5 transition-all duration-500 hover:-translate-y-2">
                            <div className="flex justify-between items-start mb-8">
                                ...
                                <span className="text-4xl font-serif text-brand-black/5 font-black">{pillar.id}</span>
                            </div>
            ...
            <Section variant="dark" className="py-24 overflow-hidden">
                <div className="text-center mb-16 space-y-4">
                    <h2 className="text-4xl md:text-5xl font-serif">Reach Across <span className="text-brand-yellow">Kenya.</span></h2>
                ...
                <div className="flex flex-wrap justify-center gap-8 max-w-5xl mx-auto">
                    {frequencies.map((f, i) => (
                        <div key={i} className="px-8 py-6 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-sm group hover:border-brand-yellow transition-colors text-center w-full sm:w-64">
                            ...
                            <span className="text-3xl font-serif text-white group-hover:text-brand-yellow transition-colors">{f.freq}</span>
                        </div>
                    ))}
                </div>
            </Section>
